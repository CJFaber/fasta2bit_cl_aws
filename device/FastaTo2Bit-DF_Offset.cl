...
static void read_inp(__global const uint16* inp, uint16* buf_in, const unsigned int size, const unsigned int offset)
{	
    __attribute__((xcl_pipeline_loop(1)))
    __attribute__((xcl_loop_tripcount(c_size, c_size)))
	read: for (unsigned int i = 0; i < size; i++){
		buf_in[i] = inp[offset + i];
	}
}

static void write_out(__global uchar16* out, uchar16* buf_out, const unsigned int size, const unsigned int offset)
{
	__attribute__((xcl_pipeline_loop(1)))
    __attribute__((xcl_loop_tripcount(c_size, c_size)))
    write: for (unsigned int i = 0 ; i < size ; i++){
        out[offset + i] = buf_out[i];
    }
}
...
void run_convert(__global const uint16* inp_dna, __global uchar16* outp_dna, const unsigned int size, const unsigned int offset)
{

	uint16 buf_in[BUFFER_SIZE];
	uchar16 buf_out[BUFFER_SIZE];

	read_inp(inp_dna, buf_in, size, offset);
	convert_fasta(buf_in, buf_out, size);
	write_out(outp_dna, buf_out, size, offset);
}

__kernel
__attribute__((reqd_work_group_size(1,1,1)))
void FastaTo2Bit_df_offset(__global const uint16* inp_dna, __global uchar16* outp_dna, const unsigned int size, const unsigned int offset)
{
	run_convert(inp_dna, outp_dna, size, offset);		
}
